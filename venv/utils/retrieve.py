...
def retrieveEntries(mp, ds, search, decryptPassword = False):
	...
	if decryptPassword and len(results)==1:
		...
		printc(f"[green][+] Extracted Password: {decrypted.decode()}[/green]")
...
def checkEntry(sitename, siteurl, email, username):
    ...
    # Membuat query pencarian berdasarkan parameter yang diberikan
    query = "SELECT * FROM pm.entries WHERE sitename = %s AND siteurl = %s AND email = %s AND username = %s"
    ...
    return result is not None  # Mengembalikan True jika entri ditemukan, False jika tidak
