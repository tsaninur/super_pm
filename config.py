...
def get_master_password():
    db = dbconfig()
    cursor = db.cursor()
    cursor.execute("SELECT masterkey_hash FROM pm.secrets")
    result = cursor.fetchone()
    db.close()

    if result:
        return result[0]  # Return the stored hash of the master password
    else:
        return None  # If no master password is found, return None


def delete():
    printc("[red][-] Deleting a config clears the device secret and all your entries from the database. This means you will lose access to all your passwords that you have added into the password manager until now. Only do this if you truly want to 'destroy' all your entries. This action cannot be undone. [/red]")
